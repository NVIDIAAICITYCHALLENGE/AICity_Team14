...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
  ...
  DIR *d;
  struct dirent *dir;
  d = opendir("test_images");
  if (d)
  {
    while ((dir = readdir(d)) != NULL)
    {
        //input=strcat(dir->d_name,".jpeg");
	//input="san_tomas_hamilton_20170605_1_00126.jpeg";
	...
	continue;
	 ...
	 //input = dir->d_name+".jpeg";
	//printf(input);

	 char src[255], dest[255];

   strcpy(dest,  "test_images/");
   strcpy(src, input);

   strcat(dest, src);

	 char txtsrc[255], txtdest[255];
char * pch;
  pch = strstr (input,".jpeg");
  strncpy (pch,"",5);
  printf("Filename_stripped:%s",input);
   strcpy(txtdest,  input);
   strcpy(txtsrc, ".txt");

   strcat(txtdest, txtsrc);
	
	

	//printf("Filename %s.\n", dest);
        image im = load_image_color(dest,0,0);
	//printf("Here");
...
rename(oldname, txtdest);
	
// rename the 0.txt >> input.txt
	//printf("Filename %s.\n", dest);
	//printf("Here3");
        if(outfile){
...
char savesrc[255], savedest[255];

   strcpy(savedest,  "predictions_");
   strcpy(savesrc, input);

   strcat(savedest, savesrc);
            //save_image(im, savedest);
#ifdef OPENCV
          //  cvNamedWindow(file2, CV_WINDOW_NORMAL); 
            if(fullscreen){
                //cvSetWindowProperty(file3, CV_WND_PROP_FULLSCREEN, CV_WINDOW_FULLSCREEN);
            }
            show_image(im, savedest);
           // cvWaitKey(0);
            cvDestroyAllWindows();
#endif
        }
